...
def add_medicalrecord(PetID):
    ...
    :return:
    """
    from ..models import medicalrecords,clinic_doctor,user
    form = FormMedicalRecords()
    clinic = get_clinic_data(current_user.ID)
    print("test",clinic_doctor.query.filter_by(CID=clinic.CID).all())
    if clinic_doctor.query.filter_by(CID=clinic.CID).all() == []:
        flash('請先新增醫師，才能進行新增病歷資料')
        return redirect(url_for('clinic_views.add_doctor'))
    form.doctor.choices = [(user.query.filter_by(UID=doctor.UID).first().name,user.query.filter_by(UID=doctor.UID).first().name) for doctor in clinic_doctor.query.filter_by(CID=clinic.CID).all()]
    print(form.doctor.choices)
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('新增病歷成功')
        return redirect(url_for('clinic_views.pet_singlemedicalrecord',MID=medicalrecord.MID))
    ...
    return render_template('add_records.html', form=form, medicationList=medicationList, action="medical",type='add',PetID=PetID)
...
def edit_medicalrecord(MID):
    ...
    if form.validate_on_submit():
        ...
        medicalrecord.CID=int(clinic.CID)
    ...
    # 利用參數action來做條件，判斷目前是新增還是編輯
    return render_template('add_records.html', form=form,MID=MID,medicationList=medicationList, medicalrecords=medicalrecord, action='edit', action1="medical",PetID=form.PetID.data)
